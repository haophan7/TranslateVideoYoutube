...
winner_surface = font.render("GAME WINNER", False, YELLOW)

screen = pygame.display.set_mode((SCREEN_WIDTH + OFFSET, SCREEN_HEIGHT + 2*OFFSET))
pygame.display.set_caption("Space Invaders")
pygame.display.set_icon(pygame.image.load("Images/spaceinvaders.png"))
...
while True:
    ...
    if game.check_for_winner() and not game_ended and not winner_displayed:
        winner_displayed = True
        winner_timer = pygame.time.get_ticks()

    if winner_displayed:
        if pygame.time.get_ticks() - winner_timer >= 5000:  # 5 seconds
            pygame.quit()
            sys.exit()
    ...
    if game.run and not game_ended:
        screen.blit(level_surface, (570, 740, 50, 50))
    elif not game.run and not game_ended:
        if winner_displayed:
            screen.blit(winner_surface, (570, 740, 50, 50))
        else:
